...
const AdminLayout = () => {
    ...
    const menuItems = [
        ...
        { name: 'Services', icon: <Briefcase size={20} />, path: '/admin/edit-services' },
        { name: 'Galerie', icon: <Camera size={20} />, path: '/admin/edit-gallery' },
        { name: 'Contact', icon: <Mail size={20} />, path: '/admin/edit-contact' },
    ];
                    ...
                    {menuItems.slice(0, 5).map((item) => (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ${location.pathname === item.path
                                ? 'bg-gold text-dark font-bold shadow-lg translate-x-1'
            ...
            <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50">
                <header className="bg-white shadow-sm p-8 flex justify-between items-center sticky top-0 z-20">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="w-10 h-10 rounded-full bg-gold text-dark flex items-center justify-center font-bold font-serif text-xl border-2 border-white shadow-md">
                            A
                        </div>
                    </div>
                </header>
